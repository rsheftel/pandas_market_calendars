...
from pandas_market_calendars.calendars.cme import CMEBondExchangeCalendar, CBOTBondExchangeCalendar
...
@pytest.mark.parametrize("cme", [CMEBondExchangeCalendar(), CBOTBondExchangeCalendar()])
def test_sunday_opens(cme):
...
@pytest.mark.parametrize("cme", [CMEBondExchangeCalendar(), CBOTBondExchangeCalendar()])
def test_2020_full_holidays(cme):
...
def test_good_fridays(cme):
    ...
    # Assert good friday 2015 is an early close at 10:15
    schedule = cme.schedule("2015-01-01", "2015-12-31")
    assert pd.Timestamp("2015-04-03") in schedule.index
    assert schedule.loc[pd.Timestamp("2015-04-03"), "market_close"] == pd.Timestamp(
        "2015-04-03", tz="America/Chicago"
    ).replace(hour=10, minute=15).tz_convert("UTC")
